...
def extract_simulation_data(path_root: str, csv_path:str) -> dict:
    ...
    for dir_name in directories:
        ...
        stats["energy"] = energy
        stats["edp"] = edp
        stats["simSeconds"] =                        _safe_search(r"simSeconds \s+([0-9.]+)", text) or 0.0
        
        #Instructions profiling.

        stats["inst_branch"] =                  _safe_search(r"system.cpu.executeStats0.numBranches\s+([0-9.eE+-]+)", text) or 0.0

        stats["inst_ALU"] =                     _safe_search(r"system.cpu.statIssuedInstType_0::IntAlu\s+([0-9.eE+-]+)", text) or 0.0

        stats["inst_load"] =                    _safe_search(r"system.cpu.statIssuedInstType_0::MemRead\s+([0-9.eE+-]+)", text) or 0.0

        stats["inst_store"] =                   _safe_search(r"system.cpu.statIssuedInstType_0::MemWrite\s+([0-9.eE+-]+)", text) or 0.0
